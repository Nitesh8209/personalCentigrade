export const project = {
    projectName: 'Automation Project',
    uiProjectName: 'Automation new Ui Project',
}

export const authStates = [
    { isAuthenticated: false, name: "Unauthenticated" },
    { isAuthenticated: true, name: "Authenticated" }
  ];

export const methodologyOptions = [
    'ACR Improved Forest Management (IFM) on Non-Federal U.S. Forestlands v1.3',
    'ACR Improved Forest Management (IFM) on Non-Federal U.S. Forestlands v2.0',
    'Biochar methodology agnostic',
    'CLEAR Methodology for Cooking Energy Transitions',
    'DAC methodology agnostic',
    'Gold standard Methodology for Metered and Measured Energy Cooking Devices v1.2',
    'Gold standard Technologies and Practices to Displace Decentralized Thermal Energy Consumption (TPDDTEC)',
    'Leak detection and repair in gas production, processing, transmission, storage and distribution systems and in refinery facilities Version 4.0.0',
    'Puro Geologically Stored Carbon Edition 2024',
    'Puro Standard Biochar Methodology Edition 2022 v3',
    'Puro Standard Enhanced Rock Weathering v2',
    'QA use only frozen ACR 1.3 test methodology',
    'VM0045 Methodology for Improved Forest Management v1.1',
    'VM0047 Afforestation, Reforestation, and Revegetation, v1.0',
    'Other methodology agnostic'
  ];

  export const projectScaleOptions = [
    'Micro (less than 1000 tCO2e)',
    'Small (1000 - 10000 tCO2e)',
    'Medium (10000 - 100000 tCO2e)',
    'Large (100000+ tCO2e)',
    'Unknown'
  ];

  export const classificationCategoryOptions = [
    'Carbon avoidance',
    'Carbon reduction',
    'Carbon removal',
    'Undefined'
  ];

  export const classificationMethodOptions = [
    'Natural - The activity claim uses natural methods (e.g. IFM)',
    'Technological - The activity claim uses technology (e.g. DAC)',
    'Both: Natural and Technological - The activity claim uses both natural and technology methods (e.g. BECCS)'
  ];

export const projectApproach = {
    items: [
        { keyName: "projectSummaryVerra", value: "test" },
        { keyName: "projectMission", value: "mission" },
        { keyName: "links", value: "Link" },
        { keyName: "projectStartYear", value: "2012" },
        { keyName: "projectEndYear", value: "2020" },
        { keyName: "creditStart", value: "2013" },
        { keyName: "creditEnd", value: "2021" },
        { keyName: "projectTimeline", value: "No" },
        { keyName: "sectoralScope", value: "Energy distribution" },
        // { keyName: "cookstoveProjectType", value: '["Improved cookstove efficiency"]'},
        { keyName: "creditIssuer", value: "Gold Standard" },
        { keyName: "standardVersion", value: "v6" },
        { keyName: "projectRegistryLink", value: "https://devfoundry.centigrade.earth/projects" },
        { keyName: "projectRegistryID", value: "ID567" },
        { keyName: "projectStatus", value: "Under validation" },
        { keyName: "approachComments", value: "No" },
        { keyName: "countries", value: '["GB","IN"]' },
        { keyName: "conflictAreas", value: "Yes" },
        { keyName: "conflictMoreInfo", value: "test" },
        { keyName: "locationComments", value: "test" },
        { keyName: "projectLocationType", value: "Single location or installation" },
        { keyName: "laborChildren", value: "Yes" },
        { keyName: "laborChildDevelopment", value: '["National laws"]' },
        { keyName: "laborWorkersRights", value: "Yes" },
        { keyName: "laborComments", value: "No comments" },
        { keyName: "nationalLawsCompliance", value: "Yes" },
        { keyName: "nationalLawsDescription", value: "test" },
        { keyName: "nationalLawsMethod", value: "Government verified" },
        { keyName: "communityFrameworks", value: "test" },
        { keyName: "carbonAuthorization", value: "Yes, I have a government concession" },
        { keyName: "carbonConservationContract", value: "Yes" },
        { keyName: "carbonIssuing", value: "Yes" },
        { keyName: "carbonIssuingYes", value: "test" },
        { keyName: "carbonRegistration", value: "Yes" },
        { keyName: "carbonRegistrationYes", value: "test" },
        { keyName: "carbonRejection", value: "Yes" },
        { keyName: "carbonRejectionYes", value: "test" },
        // { keyName: "localDisplacementAttestation", value: "Yes" },
        // { keyName: "communityIdentificationAttestation", value: "Yes" },
        // { keyName: "communityConsultAttestation", value: "Yes" },
        // { keyName: "communityEngagementAttestation", value: "No" },
        // { keyName: "benefitSharingAttestation", value: "Yes" },
        { keyName: "womenAttestation", value: "Yes" },
        { keyName: "biodiversityAttestation", value: "Yes" },
        { keyName: "waterAttestation", value: "Yes" },
        { keyName: "airAttestation", value: "Yes" },
        { keyName: "parisCompliance", value: "Fully compliant" },
        { keyName: "organizationClassification", value: "501(c)(3)" },
        { keyName: "capitalRaised", value: "20" },
        { keyName: "organizationClassificationOther", value: "No organization classification" },
        { keyName: "organizationFoundingYear", value: "2010" },
        { keyName: "ownershipStructure", value: "No structure" },
        { keyName: "governanceStructure", value: "test" },
        { keyName: "organizationIncorporation", value: "kolkata" },
        { keyName: "organizationIncorporationCountry", value: '["IN"]' },
        { keyName: "projectEmployees", value: "25" },
        { keyName: "projectLandowners", value: "2" },
        { keyName: "organizationExperiencePathway", value: "1" },
        { keyName: "organizationExperienceProjects", value: "1" },
        { keyName: "organizationExperienceDetails", value: "test" },
        { keyName: "organizationOperationYears", value: "2013" },
        // { keyName: "projectCapital", value: "test" },
        { keyName: "fundingSource", value: '["Banks and financial institutions"]' },
        { keyName: "projectRevBreakdown", value: "50 lac" },
        // { keyName: "economicDisplacement", value: "Yes" },
        { keyName: "economicDisplacementExplain", value: "test" },
        { keyName: "economicDisplacementPlan", value: "Yes" },
        // { keyName: "physicalDisplacement", value: "Yes" },
        { keyName: "physicalDisplacementExplain", value: "test" },
        { keyName: "physicalMitigation", value: "Yes" },
        { keyName: "physicalMitigationDetails", value: "test" },
        { keyName: "communityDescription", value: "test" },
        { keyName: "stakeholderIdentificationProcess", value: "test" },
        { keyName: "communityImpactDetails", value: "test" },
        { keyName: "stakeholderConsultationActual", value: "Yes" },
        { keyName: "consultationDetails", value: "test" },
        { keyName: "grievance", value: "Yes" },
        { keyName: "grievanceReport", value: "Yes" },
        { keyName: "grievanceDate", value: "2010" },
        { keyName: "economicDisplacementGrievanceUpload", value: "test" },
        { keyName: "stakeholderEngagementPlan", value: "Yes" },
        { keyName: "stakeholderEngagementDetails", value: "test" },
        { keyName: "benefitSharing", value: "Yes" },
        { keyName: "benefitSharingExplanation", value: "test" },
        { keyName: "revenueShare", value: "Yes" },
        { keyName: "moneySharing", value: "test" },
        { keyName: "payementsMade", value: "2010" },
        { keyName: "paymentsMethod", value: "cash" },
        { keyName: "fairTradeMinimumPrice", value: "200000" },
        { keyName: "smartContracts", value: "No" },
        // { keyName: "deiIssue", value: "Yes" },
        { keyName: "deiConsult", value: "Yes" },
        // { keyName: "deiDetails", value: "test" },
        // { keyName: "health", value: "Yes" },
        { keyName: "deiBenefitSharing", value: "test" },
        { keyName: "womenEmpowerment", value: "Yes" },
        { keyName: "deiMonitoringPlan", value: "test" },
        { keyName: "exploitation", value: "Yes" },
        // { keyName: "deiExploitationDetails", value: "test" },
        { keyName: "womenRights", value: "Yes" },
        { keyName: "deiWomen", value: "Yes" },
        { keyName: "womenConsultation", value: "Yes" },
        { keyName: "communityExists", value: "Yes" },
        { keyName: "dataSteward", value: "Yes" },
        { keyName: "communityConsultation", value: "Yes" },
        { keyName: "landRightsRec", value: "Yes" },
        { keyName: "carbonContractLength", value: "test" },
        { keyName: "contractPublic", value: "test" },
        { keyName: "contractPenalty", value: "test" },
        { keyName: "specialMeasure", value: "Yes" },
        { keyName: "specialMeasureDetails", value: "test" },
        { keyName: "benefitSharingCommunities", value: "Yes" },
        { keyName: "compensationType", value: '["Monetary incentives - Community funds"]' },
        { keyName: "iplcOwnershipPercent", value: "80" },
        { keyName: "compensationPercent", value: "90" },
        { keyName: "paymentsFrequency", value: "Annually" },
        { keyName: "airMonitoringPlan", value: "Yes" },
        { keyName: "airMonitoringDomains", value: '["Indoor air pollution"]' },
        { keyName: "airMonitoringDescription", value: "test" },
        { keyName: "airMonitoringBaseline", value: "Yes" },
        { keyName: "airBaselineDescription", value: "test" },
        { keyName: "airMonitoringPollutants", value: '["NOx"]' },
        { keyName: "exposureMonitoringConditional", value: "Yes" },
        { keyName: "airMonitoringBaselineFrequency", value: "test" },
        { keyName: "airMonitoringMechanism", value: "test" },
        { keyName: "exposureMonitoringMechanism", value: "test" },
        { keyName: "indoorAirDetails", value: "test" },
        { keyName: "airVerification", value: "Yes" },
        { keyName: "airVerificationBody", value: "test" },
        { keyName: "ecosystemBiome", value: '["Tropical and subtropical moist broadleaf forests"]' },
        { keyName: "ecosystemCategorization", value: '["Collapsed"]' },
        { keyName: "biomeDescription", value: "test" },
        { keyName: "biomeRiskCategory", value: "test" },
        { keyName: "biodiversityMonitoring", value: "Yes" },
        { keyName: "biodiversityMonitoringProcess", value: "test" },
        { keyName: "biodiversityImpactDrivers", value: "test" },
        { keyName: "soilEffects", value: "test" },
        // { keyName: "speciesNumber", value: "12" },
        { keyName: "threatenedSpecies", value: "test" },
        { keyName: "biodiversityLegal", value: "test" },
        { keyName: "biodiversitySpecies", value: "Yes" },
        { keyName: "speciesSelected", value: '["Sentinel species"]' },
        { keyName: "speciesMonitoring", value: "Quarterly" },
        { keyName: "specieisSamplingDensity", value: "60%" },
        { keyName: "speciesCollectionAudit", value: "Yes" },
        { keyName: "specieisCollectionMethodDetails", value: "test" },
        { keyName: "speciesCollectionResults", value: "test" },
        { keyName: "biodiversityAudit", value: "test" },
        { keyName: "waterMonitoringPlan", value: "Yes" },
        { keyName: "waterImpact", value: "Yes" },
        { keyName: "waterMonitorIntention", value: "test" },
        { keyName: "expectedWaterImpacts", value: '["Project impacts water quality"]' },
        { keyName: "waterLocalDetails", value: "test" },
        { keyName: "waterBaselineCondition", value: "Yes" },
        { keyName: "waterBaselineDescription", value: "test" },
        { keyName: "waterMonitoringFrequency", value: "test" },
        { keyName: "waterStewardship", value: '["Land conservation and restoration - Land conservation (protection and preservation)"]' },
        { keyName: "waterVWBConditional", value: "Yes" },
        { keyName: "waterVWBIndicators", value: '["Avoided runoff"]' },
        { keyName: "waterCompIndicators", value: '["Crop yield - Mass per area over time"]' },
        { keyName: "waterMonitoringIndicatorDetails", value: "test" },
        // {keyName: "waterDependency", value: "test"},
        { keyName: "waterVerification", value: "Yes" },
        { keyName: "waterVerificationBody", value: "test" },
    ]
};




export const FileType = [
    { projectFileType: 'projectMedia' },
    { projectFileType: 'pdd' },
    { projectFileType: 'geographicLocation' },
    { projectFileType: 'laborUpload' },
    { projectFileType: 'nationalLawsUpload' },
    { projectFileType: 'carbonCreditOwnership' },
    { projectFileType: 'carbonUpload' },
    { projectFileType: 'parisLetter' },
    { projectFileType: 'organizationStructureUpload' },
    { projectFileType: 'projectCostRevUpload' },
    { projectFileType: 'physicalUpload' },
    { projectFileType: 'expectedImpactsDocument' },
    { projectFileType: 'consultationDocuments' },
    { projectFileType: 'grievanceSupport' },
    { projectFileType: 'stakeholderEngagementUpload' },
    { projectFileType: 'benefitUpload' },
    { projectFileType: 'deiBenefitSharingUpload ' },
    { projectFileType: 'airBaselineEvidence' },
    { projectFileType: 'indoorAirEvidence' },
    { projectFileType: 'biodiversityEvidence' },
    { projectFileType: 'waterUpload' },
]


export const forcastData = {
    items: [
        { keyName: "ifmRequirementsVerra", value: '["The project area qualifies as a \\"forest remaining as forest\\"","The project activity does not reduce the frequency and/or intensity of timber harvest and is not subject to any pre-existing legal encumbrance specifically restricting timber harvest","The project does not include Wetland Restoration and Conservation (WRC) activities"]' },
        { keyName: "baselineStartYear", value: "2015" },
        { keyName: "baselineEndYear", value: "2025" },
        { keyName: "dynamicBaseline", value: "Yes" },
        { keyName: "dynamicBaselineSummary", value: "test" },
        { keyName: "stratification", value: "Yes" },
        { keyName: "stratificationProcedure", value: "test" },
        { keyName: "ifmVerraBslPlotSourcing", value: "test" },
        { keyName: "ifmVerraBslPlotReqs", value: "[\"Baseline plots are located outside of the project area\",\"Baseline plots are selected from a donor pool that uses unbiased, representative sampling at a regional scale\"]" },
        { keyName: "donorPoolSize", value: "Yes" },
        { keyName: "projectActivityVerra", value: "test" },
        { keyName: "harvestingPlanOverview", value: "harvesting plan test" },
        { keyName: "verraExtraGhgSources", value: '["Nitrogen fertilizer","Burning of biomass"]' },
        { keyName: "ifmVerraProjectPlots", value: "200" },
        { keyName: "baselinePlotSourcingCovariates", value: "100" },
        { keyName: "continuousCovariates", value: "Yes" },
        { keyName: "matching", value: "Yes" },
        { keyName: "deviations", value: "test information about methodology deviations" },
        { keyName: "sensitiveInformation", value: "test about the commercially sensitive information" },
        { keyName: "projectDescriptionVerra", value: "test about the Project technology and equipment overview" },
        { keyName: "technologyManufacturer", value: "any technology for test" },
        { keyName: "controlSiteComments", value: "No comments" },
        { keyName: "priorConditionsVerra", value: "No physical conditions" },
        { keyName: "systemBoundaries", value: '["Aboveground tree biomass"]' },
        // { keyName: "systemBoundariesExplain ", value: "test Carbon pools and sources additional comments" },
        { keyName: "VerraForestryEmissionSources", value: "[\"N2O emissions from nitrogen-containing soil amendments or from decomposition of plant materials with fixed nitrogen\"]" },
        { keyName: "emissionSourcesComments", value: "test Emission sources additional comments" },
        { keyName: "VerraForestryUpstreamEmissions", value: "test Overview of upstream emissions" },
        { keyName: "baselineComments", value: "test Forecasted baseline and project carbon stocks overview" },
        { keyName: "leakageEstimatesConditional", value: "Yes" },
        { keyName: "LeakageCategorySelection", value: "[\"Activity-shifting leakage\",\"Market leakage\"]" },
        { keyName: "leakageDescription", value: "test" },
        // { keyName: "ifmVerraleakageFactorConditional", value: "No" },
        { keyName: "ifmVerraleakageFactorConditional", value: "10%" },
        { keyName: "leakageComments", value: "test" },
        { keyName: "uncertaintyEstimatesConditional", value: "Yes" },
        { keyName: "uncertaintyComments", value: "test" },
        { keyName: "bufferEstimatesConditional", value: "Yes" },
        { keyName: "nonPermanenceTool", value: "No" },
        { keyName: "bufferComments", value: "test" },
        { keyName: "forecastConditional", value: "Yes" },
        { keyName: "forecastAssumptions", value: "test" },
        { keyName: "forecastComments", value: "test" },
        { keyName: "monitoringPlan", value: "test MRV protocol and monitoring plan overview" },
        { keyName: "ifmVerraMonitoringReqs", value: '["Definition of the accounting boundary"]' },
        { keyName: "dataParameters", value: "test Parameters monitored" },
        { keyName: "dataProcess", value: "test Data processing and storage procedures" },
        { keyName: "monitoringTimeline", value: "60" },
        { keyName: "ifmVerraDatabaseReqs", value: "test Database requirements for project and control plots" },
        { keyName: "permanenceNumber", value: "10-100 years" },
        { keyName: "carbonStorageDetails", value: "test" },
        // { keyName: "reversalRisk", value: '["Extreme weather (e.g. drought)","Pest and disease"]' },
        { keyName: "reversalRiskOther", value: "test" },
        { keyName: "riskManagementPlan", value: "Yes" },
        { keyName: "riskPlan", value: "test" },
        { keyName: "durabilityInsurance", value: '["None"]' },
        { keyName: "insuranceOwner", value: "test" },
        { keyName: "additionalityApproachUsed", value: "[\"First-of-its-kind\",\"Financial additionality\"]" },
        { keyName: "validationAdditionality", value: "Yes" },
        { keyName: "validationAdditionalityThirdParty", value: "test" },
        { keyName: "firstOfItsKind", value: "Yes" },
        { keyName: "firstOfItsKindAnalysis", value: "test" },
        { keyName: "initialAdditionality", value: "Yes" },
        { keyName: "investmentMethod", value: "[\"Simple cost analysis\"]" },
        { keyName: "simpleCostAdditionality", value: "Yes" },
        { keyName: "simpleCostAnalysis", value: "2000 lac" },
        { keyName: "projectRevenues", value: "1000 lac" },
        { keyName: "costs", value: "100 lac" },
        { keyName: "validationBody", value: "test Validation body" },
        { keyName: "validationDate", value: "20-11-2024" },
    ]
}

export const forcastFileType = [
    { projectFileType: 'ifmRequirementsVerraDocumentation' },
    { projectFileType: 'ifmVerraProjectPlotsDocumentation' },
    { projectFileType: 'matchingWorkbook' },
    { projectFileType: 'generalDocumentUploac' },
    { projectFileType: 'technicalUpload' },
    { projectFileType: 'projectActivityLand' },
    { projectFileType: 'ifmVerraBslPlotMap' },
    { projectFileType: 'controlSite' },
    { projectFileType: 'baselineForecastWorkbook' },
    { projectFileType: 'projectForecastWorkbook' },
    { projectFileType: 'leakageForecastWorkbook' },
    { projectFileType: 'bufferForecastWorkbook' },
    { projectFileType: 'riskToolWorkbook' },
    { projectFileType: 'monitoringWorkbook' },
    { projectFileType: 'riskManagementPlanUpload' },
    { projectFileType: 'firstOfItsKindUpload' },
    { projectFileType: 'simpleCostUpload' },
    { projectFileType: 'validationUpload' },
]

export const forcastSeries = [
    {
        indexOrder: 0,
        name: "estimatedBaseline",
        notes: "Estimated baseline carbon stocks",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "100"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "200"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "300"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "400"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "500"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "600"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "700"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "700"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "900"
            }
        ]
    },
    {
        indexOrder: 1,
        name: "estimatedProjectStocks",
        notes: "Estimated project carbon stocks",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "500"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "700"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "500"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "600"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "800"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "400"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "1000"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "1000"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "1200"
            }
        ]
    },
    {
        indexOrder: 0,
        name: "estimatedLeakage",
        notes: "Estimated leakage carbon quantities in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "40"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "50"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "40"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "10"
            }
        ]
    },
    {
        indexOrder: 1,
        name: "estimatedLeakageFactor",
        notes: "Estimated leakage factor (%)",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "4"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "6"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "7"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "4"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "8"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "7"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "5"
            }
        ]
    },
    {
        indexOrder: 0,
        name: "estimatedBufferCarbon",
        notes: "Estimated buffer carbon quantities in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "200"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "300"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "150"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "200"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "400"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "300"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "500"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "400"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "600"
            }
        ]
    },
    {
        indexOrder: 1,
        name: "estimatedBufferNPR",
        notes: "Estimated NPR% (non-permanence risk rating)",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "8"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "18"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "28"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "30"
            }
        ]
    },
    {
        indexOrder: 0,
        name: "estimatedNetReductions",
        notes: "Estimated net reductions in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "40"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "50"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "40"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "50"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "20"
            }
        ]
    },
    {
        indexOrder: 1,
        name: "estimatedNetRemovals",
        notes: "Estimated net removals in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "18"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "14"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "10"
            }
        ]
    },
    {
        indexOrder: 2,
        name: "estimatedNetReductionRemoval",
        notes: "Estimated net reductions and removals in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "35"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "35"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "35"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "15"
            }
        ]
    }
]



export const actualsData = {
    items: [
        { keyName: "ifmVerraProjUpdateParent", value: "[\"Stratification or stands\"]" },
        { keyName: "ifmVerraProjUpdateStands", value: "test" },
        { keyName: "mrvProtocol", value: "Yes" },
        { keyName: "mrvResultsLinks", value: "test" },
        { keyName: "mrvProvider", value: "test" },
        { keyName: "compensation", value: "Yes" },
        { keyName: "compensationMechanismDescription", value: "test" },
        { keyName: "reversalEvent", value: "Yes" },
        { keyName: "reversalEventType", value: "[\"Natural causes\"]" },
        { keyName: "reversalEventTypeOther", value: "test" },
        { keyName: "reversalEventStatus", value: "In process" },
        { keyName: "bufferActualsConditional", value: "Yes" },
        { keyName: "additionalCommentsBuffer", value: "test" },
        { keyName: "verificationBody", value: "test" },
        { keyName: "verificationDate", value: "20-11-2024" },
        { keyName: "reportingPeriod", value: "20" },
        { keyName: "creditsIssuedPeriod", value: "10000" },
        { keyName: "verificationResampling", value: "test" },
        { keyName: "estimatedPriceMin", value: "10000" },
        { keyName: "estimatedPriceMax", value: "400000" },
        { keyName: "creditSalesStage", value: "[\"Spot\",\"Offtake agreement\"]" },
        { keyName: "carbonCreditBuyer", value: "test" },
    ]
}

export const actualsFileType = [
    { projectFileType: 'ifmVerraProjUpdateStandsUpload' },
    { projectFileType: 'mrvResultsDocs' },
    { projectFileType: 'compensationDetails' },
    { projectFileType: 'reversalEventTypeLink' },
    { projectFileType: 'reversalEventStatusLink' },
    { projectFileType: 'projectWorkbook' },
    { projectFileType: 'leakageWorkbook' },
    { projectFileType: 'uncertaintyWorkbook' },
    { projectFileType: 'bufferWorkbook' },
    { projectFileType: 'riskToolWorkbookUpdate' },
    { projectFileType: 'actualsWorkbook' },
    { projectFileType: 'verificationUpload' },
]

export const acutalSeriesData = [
    {
        indexOrder: 0,
        name: "actualProjectStocks",
        notes: "Actual project carbon stocks",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "100"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "200"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "300"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "400"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "500"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "600"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "700"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "600"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "400"
            }
        ]
    },
    {
        indexOrder: 0,
        name: "actualLeakageCarbon",
        notes: "Actual leakage carbon quantities in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "40"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "20"
            }
        ]
    },
    {
        indexOrder: 1,
        name: "actualLeakageFactor",
        notes: "Actual Leakage factor (%)",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "5"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "12"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "14"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "8"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "9"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "5"
            }
        ]
    },
    {
        indexOrder: 0,
        name: "actualUncertaintyPercent",
        notes: "Actual total uncertainty (%)",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "24"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "40"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "35"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "29"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "19"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "14"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "23"
            }
        ]
    },
    {
        indexOrder: 1,
        name: "actualUncertaintyCarbon",
        notes: "Actual uncertainty carbon quantities in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "16"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "23"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "54"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "12"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "45"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "23"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "45"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "24"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "35"
            }
        ]
    },
    {
        indexOrder: 0,
        name: "updateBufferCarbon",
        notes: "Actual buffer carbon quantities in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "14"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "23"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "27"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "29"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "35"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "38"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "40"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "24"
            }
        ]
    },
    {
        indexOrder: 1,
        name: "updateNPR",
        notes: "Actual NPR% (non-permanence risk rating)",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "17"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "35"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "38"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "23"
            }
        ]
    },
    {
        indexOrder: 0,
        name: "actualNetReductions",
        notes: "Actual net reductions in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "32"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "35"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "27"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "17"
            }
        ]
    },
    {
        indexOrder: 1,
        name: "actualNetRemovals",
        notes: "Actual net removals in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "16"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "24"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "19"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "26"
            }
        ]
    },
    {
        indexOrder: 2,
        name: "actualNetReductionsRemovals",
        notes: "Actual net reductions and removals in tCO2e",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "8"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "15"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "8"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "26"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "24"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "15"
            }
        ]
    },
    {
        indexOrder: 0,
        name: "creditsAvailable",
        notes: "Available credits",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "29"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "27"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "16"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "39"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "38"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "35"
            }
        ]
    },
    {
        indexOrder: 1,
        name: "exAnteAvailable",
        notes: "Credits ex ante available",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "9"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "23"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "28"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "37"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "34"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "48"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "58"
            }
        ]
    },
    {
        indexOrder: 2,
        name: "creditsSold",
        notes: "Credits sold",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "7"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "18"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "19"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "26"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "37"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "28"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "50"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "47"
            }
        ]
    },
    {
        indexOrder: 3,
        name: "creditsOnHold",
        notes: "On-hold credits",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "19"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "17"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "26"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "40"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "38"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "38"
            }
        ]
    },
    {
        indexOrder: 4,
        name: "creditsOfftakes",
        notes: "Offtake agreement credits sold",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "9"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "20"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "16"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "28"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "30"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "39"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "40"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "60"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "26"
            }
        ]
    },
    {
        indexOrder: 5,
        name: "creditsForward",
        notes: "Forward credits sold",
        seriesData: [
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2013",
                indexOrder: 2013,
                value: "10"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2014",
                indexOrder: 2014,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2015",
                indexOrder: 2015,
                value: "26"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2016",
                indexOrder: 2016,
                value: "29"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2017",
                indexOrder: 2017,
                value: "39"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2018",
                indexOrder: 2018,
                value: "25"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2019",
                indexOrder: 2019,
                value: "58"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2020",
                indexOrder: 2020,
                value: "49"
            },
            {
                dataTypeName: "FLOAT_NUMBER",
                indexName: "2021",
                indexOrder: 2021,
                value: "50"
            }
        ]
    },

]



export const EnhancementsData = {
    items: [
        { keyName: "kd1Title", value: "test1" },
        { keyName: "kd1Details", value: "test1 details" },
        { keyName: "kd2Title", value: "test2" },
        { keyName: "kd2Details", value: "test2 details" },
        { keyName: "kd3Title", value: "test3" },
        { keyName: "kd3Details", value: "test3 details" },
        { keyName: "sdg", value: "[\"SDG 1: No poverty\",\"SDG 2: Zero hunger\",\"SDG 3: Good health and well-being\",\"SDG 4: Quality education\"]" },
        { keyName: "sdgDetails", value: "test SDG details" },
        { keyName: "sdgVerification", value: "Yes" },
        { keyName: "sdgVerificationBody", value: "test verification body" }
    ]
}

export const EnhancementsFileType = [
    { projectFileType: 'kd1Media' },
    { projectFileType: 'kd2Media' },
    { projectFileType: 'kd3Media' },
    { projectFileType: 'sdgUpload' },
    { projectFileType: 'sdgVerificationEvidence' },
]

export const organizationtypeData = [
    {
        name: "FORM_BASIC",
        roleTypeId: 15,
        description: "The ability to form projects as a pro",
    },
    {
        name: "FORM_CREATOR",
        roleTypeId: 17,
        description: "The ability to create forms",
    }
]